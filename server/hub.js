...
capsServer.on('connection', (socket)=>{
  ...
  socket.on(eventPool[0], (payload) =>{
    ...
    if (toBePickedUp){
      ...
      let newStoreQueue = new DeliveryQueue();
      newStoreQueue.store(payload.orderId, payload);
      pickupQue.store(payload.store, newStoreQueue);
    }
    console.log('STORING ', pickupQue);
  ...
  socket.on(eventPool[2], (payload) =>{
    ...
    console.log('Payload to be move from PICK UP to DELIVERED', payload, deliveredQueue);
  ...
  socket.on('getAll', (payload)=>{
    ...
    let orderKeys = Object.keys(orders.data);
